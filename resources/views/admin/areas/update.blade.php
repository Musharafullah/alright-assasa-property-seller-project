...
line-height: 10px;
    ...
    <div class="row mb-3">
        <div class="col-md-8">
            <h1 class="h3 d-inline align-middle" style="font-size: 30px;"> <b>Update  Area</b> </h1>
        ...
        <div class="row">
            <div class="container m-auto" style="margin-top:20px">
                <form class="myform " action="{{ url('/admin/areas/' . $area->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-6 mb-2 px-4">
                            ...
                            <input type="text" name='name' value="{{ $area->name }}" class="form-control" id="exampleInputArea">
                                ...
                                @foreach ($area->phases as $phase)
                                    @if ($loop->first)
                                        <div class="d-flex align-items-center gap-4">
                                            <input type="hidden" name="phases_id[]" value="{{ $phase->id }}">
                                    ...
                                    @else
                                        <div class="d-flex align-items-center gap-4">
                                            <input type="hidden" name="phases_id[]" value="{{ $phase->id }}">
                                            <input type="text" name='phases[]' class="form-control" value="{{ $phase->title }}" required>
                                        </div>
                                    @endif
                                @endforeach
                            </div>
...
@push('scripts')
    <script>
        function addNewPhase() {
            $("#phases-array").append(`
                <div class="d-flex align-items-center gap-4">
                    <input type="text" name='phases[]' class="form-control" required>
                    <button onclick="$(this).parent().remove()" class="btn " type="button" style=" background:linear-gradient(to bottom,#DA0B0B,#9A2929)"><svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
