...
<input   type="{{ isset($type) ? $type : 'text' }}" name="{{ $name }}" @if (isset($inputValue) && $inputValue) value="{{ $inputValue }}" @endif class="form-control" id="input{{ $name }}"
       @if (isset($required) && $required == true) required @endif @if (isset($size) && $size) size="{{ $size }}" @endif @if (isset($accept) && $accept) accept="{{ $accept }}" @endif @if (isset($readonly) && $readonly) readonly="{{ $readonly }}" @endif>
